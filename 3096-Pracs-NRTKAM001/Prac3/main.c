...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  poll1 = pollADC();
	  __HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_4, ADCtoCRR(poll1));
	  sprintf(buffer, "%f\r\n", ((poll1*100)/4095.0));
	  ...
	  HAL_Delay (DELAY1); // wait for 500 ms
...
void EXTI0_1_IRQHandler(void)
{
	...
	//DEBOUNCING
	int tick_now = HAL_GetTick();

	if(tick_now - tick_prev >= 10)
...
uint32_t pollADC(void){
   ...
   // Get ADC value
	HAL_ADC_Start(&hadc);
	HAL_ADC_PollForConversion(&hadc, DELAY1);
